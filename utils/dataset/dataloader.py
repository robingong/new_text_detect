...
from utils.dataset.label_maker import make_border_map, make_score_map
from utils.dataset.generator_enqueuer import GeneratorEnqueuer
from utils.dataset.img_aug import crop_area, det_aug
from utils.utils import resize_img, load_ctw1500_labels, load_icdar_labels


def load_labels(gt_path, data_name='icdar'):
    if data_name == 'ctw1500':
        return load_ctw1500_labels(gt_path)
    elif data_name == 'icdar':
        return load_icdar_labels(gt_path)
...
if __name__ =='__main__':
    img_dir = '/data/文本检测/text_detection_idcar/image'
    label_dir = '/data/文本检测/text_detection_idcar/txt'
    ...
    for img_name in img_list:
        try:
            ...
            #print(img_input)
            #print(score_map[:, :, np.newaxis])
            #print(score_mask[:, :, np.newaxis])
            #print(threshold_map[:, :, np.newaxis])
            #print(thresh_mask[:, :, np.newaxis])
        ...
        h, w, _ = img.shape
        polys, tags = load_labels(os.path.join(label_dir, os.path.splitext(img_name)[0] + '.txt'))
        #img_input = np.zeros([cfg.TRAIN.IMG_SIZE, cfg.TRAIN.IMG_SIZE, 3], dtype=np.float32)
